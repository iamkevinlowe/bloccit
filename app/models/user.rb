class User < ActiveRecord::Base
  ...
  def self.top_rated
    ...
    .group('users.id') # Instructs the database to group the results so taht each user will be returned in a distinct row
    .order('rank DESC') # Instructs the database to order the results in descending order, by the rank taht we created in this query. (rank = comment count + post count)
  end
end